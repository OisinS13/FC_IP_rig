...
SerialPIO UART3(12, 13, 32);
SerialPIO UART4(14, 15, 32);
...
void setup() {
  ...
  Serial2.setRX(9);  //Set UART pins for UART1, to be used for Interface connection
  Serial2.setTX(8);
...
void loop() {
  ...
  if (CVM.available()) {  //EDITME change to while?
    char ID = CVM.currentPacketID();

    if (ID == 'd') {  //Data frame
      Receive_data_frame(0x01);
    }
  ...
  if ((*DataPtr[0]).Data_source_flags & 0x10) {  //If the "To-be-saved" bit is set
    ...
    j += sprintf(&Data_to_file[j], "%f,", (*DataPtr[0]).Process_data.CTHD_FLW_target);  //Append a reading, and then a delimeter
    j += sprintf(&Data_to_file[j], "%u,", (*DataPtr[0]).Process_data.CTHD_FLW_output);  //Append a reading, and then a delimeter
    for (int i = 0; i < 2; i++) {
    ...
    for (int i = 0; i < 3; i++) {
      j += sprintf(&Data_to_file[j], "%f,", (*DataPtr[0]).Process_data.Coolant_T_readings[i]);  //Append a reading, and then a delimeter
    }
    j += sprintf(&Data_to_file[j], "%u,", (*DataPtr[0]).Process_data.Fan_setting);  //Append a reading, and then a delimeter
    for (int i = 0; i < 8; i++) {
    ...
    j += sprintf(&Data_to_file[j], "%f,", (*DataPtr[0]).Process_data.O2_conc);                        //Append a reading, and then a delimeter
    ...
    // (*DataPtr[0]) = Temporary;
    DataFrameStruct *DataPtr_temp = DataPtr[0];
    for (int i = 0; i < (BufferSize - 1); i++) {
      DataPtr[i] = DataPtr[i + 1];
    }
    DataPtr[BufferSize] = DataPtr_temp;
  }
}
