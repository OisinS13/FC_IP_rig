...
void loop() {
  ...
  for (int i = 0; i < 5; i++) {
    Outgoing_data.P_readings[i] = ((analogRead(P_sig_pins[i]) * 1000000) - 102400000) / 102400;  //Convert analog reading to pressure in mBar
  }
  ...
  if (Mode == 0) {
    ...
    //EDITME add anode pressure based flow control
